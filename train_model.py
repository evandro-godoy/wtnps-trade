...
def train_all_models(config_path: str = 'configs/main.yaml'):
    ...
    # Itera diretamente sobre a lista de ativos. Usa get('assets', []) para default vazio. # <-- CORREÇÃO AQUI
    for asset_config in config.get('assets', []):
        # Extrai o símbolo do ticker de dentro da configuração do ativo atual # <-- CORREÇÃO AQUI
        asset_symbol = asset_config.get('ticker')
        if not asset_symbol:
             logger.warning("Configuração de ativo encontrada sem 'ticker'. Pulando.")
             continue # Pula esta entrada se não houver ticker

        # A partir daqui, 'asset_symbol' e 'asset_config' estão definidos corretamente
        if not asset_config.get('enabled', True): # Verifica 'enabled' dentro do loop # <-- CORREÇÃO AQUI
        ...
        # Usa o asset_symbol (ticker principal) se 'ticker' não estiver na seção 'data'
        ticker_data = train_cfg.get('ticker', asset_symbol)

        if not all([train_cfg.get('start_date'), train_cfg.get('end_date'), train_cfg.get('timeframe_model')]):
        ...
        try:
            ...
            if market_data.empty:
                logger.warning(f"Nenhum dado retornado para {ticker_data} no período especificado. Pulando.")
                continue
        ...
        try:
            ...
            # Garante que todas as features existam antes de selecionar
            missing_features = [f for f in feature_names if f not in data_with_features.columns]
            if missing_features:
                 logger.error(f"Features ausentes após define_features para {asset_symbol}: {missing_features}. Verifique a estratégia {strategy_class_name}.")
                 continue
            ...
            if combined.empty:
                logger.warning(f"Após adicionar features/target e remover NaNs, não restaram dados para {ticker_data}. Pulando.")
                continue
            ...
            logger.error(f"Erro ao preparar features/target para {ticker_data}: {e}", exc_info=True)
            continue
        ...
        try:
            ...
            logger.error(f"Erro durante a definição ou treino do modelo para {asset_symbol}: {e}", exc_info=True)
            continue

        # --- Salvamento do Modelo (Usando a Estratégia) ---
        try:
            # Usa o asset_symbol (chave do YAML, ex: WDO$) para nomear o arquivo
