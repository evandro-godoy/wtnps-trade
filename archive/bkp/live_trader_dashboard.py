...
class LiveTradingDashboard(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("WtnpsTrade Live Trader Simulation Dashboard v1.1")
        ...
        try:
            ...
            self.initialization_thread.start()
             ...
             log.critical(f"Erro fatal LiveTrader: {e}", exc_info=True); messagebox.showerror("Erro Inicialização", f"{e}"); self.destroy(); return
        ...
        refresh_controls_frame = ttk.Frame(self.main_frame); refresh_controls_frame.pack(side=tk.TOP, fill=tk.X, pady=(0, 10))
        self.timer_label = ttk.Label(refresh_controls_frame, text="Próxima atualização em: 60s"); self.timer_label.pack(side=tk.LEFT, padx=5)
        ...
        # Monitor de Mercado
        self.market_data_frame = ttk.LabelFrame(self.main_frame, text="Monitor de Mercado", padding="10"); self.market_data_frame.pack(side=tk.TOP, fill=tk.X, pady=5)

        # Frame Simulação
        sim_frame = ttk.LabelFrame(self.main_frame, text="Simulação de Ciclo Único (LiveTrader Engine)", padding="10"); sim_frame.pack(side=tk.TOP, fill=tk.X, pady=5)
        self.asset_selection_frame = ttk.Frame(sim_frame); self.asset_selection_frame.pack(side=tk.TOP, fill=tk.X, pady=5)
        self.simulate_button = ttk.Button(sim_frame, text="Executar Simulação de Ciclo Único", command=self._trigger_simulation, state=tk.DISABLED); self.simulate_button.pack(side=tk.TOP, pady=10)

        # Frame Logs
        log_frame = ttk.LabelFrame(self.main_frame, text="Log da Simulação", padding="10"); log_frame.pack(side=tk.BOTTOM, fill=tk.BOTH, expand=True, pady=5)
        self.log_text = scrolledtext.ScrolledText(log_frame, wrap=tk.WORD, height=15, font=("Consolas", 9)); self.log_text.pack(expand=True, fill=tk.BOTH)
