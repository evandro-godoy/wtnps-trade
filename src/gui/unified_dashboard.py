...
class UnifiedDashboard(tk.Tk):
    ...
    def _create_simulation_controls(self, parent):
        ...
        now_local_str = datetime.now(self.local_tz).strftime("%Y-%m-%d %H:%M:00"); self.sim_datetime_var = tk.StringVar(value=now_local_str); self.sim_datetime_entry = ttk.Entry(frame, textvariable=self.sim_datetime_var, width=20)
    ...
    def _run_simulation_click(self):
        ...
        if not self.is_simulation_engine_initialized or not self.simulation_engine:
             messagebox.showerror("Erro", "Motor de Simulação não inicializado."); return
        asset = self.sim_asset_var.get(); tf = self.sim_tf_var.get(); dt_str = self.sim_datetime_var.get()
        if not asset: messagebox.showwarning("Atenção", "Selecione um ativo."); return
        try: dt_local = datetime.strptime(dt_str, "%Y-%m-%d %H:%M:%S")
        except ValueError: messagebox.showerror("Erro", f"Formato Data/Hora inválido: {dt_str}"); return
        # Converte para aware no timezone local
        try: dt_local_aware = self.local_tz.localize(dt_local, is_dst=None) # is_dst=None ajuda com horários ambíguos
        except (pytz.exceptions.NonExistentTimeError, pytz.exceptions.AmbiguousTimeError) as e:
             messagebox.showerror("Erro Fuso Horário", f"Data/Hora inválida ou ambígua: {e}"); return
    ...
    def _update_market_monitor(self):
        ...
        for asset_symbol in self.live_asset_tickers:
            ...
            if tick and tick.time > 0: # Verifica se o tick é válido
                ...
                tick_time_utc = datetime.utcfromtimestamp(tick.time).replace(tzinfo=pytz.utc)
